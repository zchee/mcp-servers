    ...
    for section in primary_content:
        ...
        if kind == "declarations":
            ...
            for param in params:
                name = param.get("name", "")
                content += f"- `{name}`: "
                content += " ".join(item.get("text", "") for item in param.get("content", []))
                content += "\n"
            content += "\n"
        elif kind == "content":
            content += format_content_section(section)
...
def format_platform_availability(json_data: dict[str, Any]) -> str:
    ...
    for platform in platforms:
        ...
        content += f"- **{name}**: "
        if introduced:
            content += f"Introduced in {introduced}"
        if deprecated:
            content += f", Deprecated in {deprecated}"
        content += "\n"
...
def format_related_apis_section(related_apis: list[dict[str, str]]) -> str:
    ...
    for api in related_apis:
        content += f"- [{api['title']}]({api['url']}) ({api['relationship']})\n"
    content += "\n"
    return content
...
def format_references_section(references: list[dict[str, str]]) -> str:
    ...
    for ref in references:
        content += f"- [{ref['title']}]({ref['url']}) ({ref['type']})\n"
        if ref.get("abstract"):
            content += f"  {ref['abstract']}\n"
    content += "\n"
    return content
...
def format_similar_apis_section(similar_apis: list[dict[str, str]]) -> str:
    ...
    content = "## Similar APIs\n\n"
    for api in similar_apis:
        content += f"- [{api['title']}]({api['url']}) ({api['category']})\n"
    content += "\n"
    return content
...
def format_platform_analysis_section(analysis: dict[str, Any]) -> str:
    ...
    content += f"**Supported Platforms:** {analysis['supportedPlatforms']}\n\n"

    if analysis["betaPlatforms"]:
        content += f"**Beta Platforms:** {', '.join(analysis['betaPlatforms'])}\n\n"

    if analysis["deprecatedPlatforms"]:
        content += f"**Deprecated Platforms:** {', '.join(analysis['deprecatedPlatforms'])}\n\n"

    content += f"**Cross-Platform:** {'Yes' if analysis['crossPlatform'] else 'No'}\n\n"
